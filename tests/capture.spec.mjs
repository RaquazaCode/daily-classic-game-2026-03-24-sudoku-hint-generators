...
test("captures deterministic sudoku run", async ({ page }) => {
  ...
  await page.getByRole("gridcell", { name: "Cell r1 c1" }).click();
  await page.keyboard.press("5");
  await page.getByRole("gridcell", { name: "Cell r1 c2" }).click();
  ...
  expect(verification.hintCharges).toBeGreaterThanOrEqual(0);
  ...
  const actionsStart = {
    ...
    buttons: ["enter", "left_mouse_button", "5"],
    mouse_x: 220,
    mouse_y: 250,
    frames: 4,
  };
  ...
  fs.writeFileSync("artifacts/playwright/actions-build-battle.json", `${JSON.stringify(actionsHint, null, 2)}\n`);
  ...
  fs.writeFileSync("artifacts/playwright/clip-hint-generator.gif", "placeholder\n");
